...
> **Demo plane (Voice Live + Foundry Agent Service, [ADR-0004](./adr/ADR-0004-voice-live-foundry-agent.md)).** When the virtual-pilot brain runs as a Foundry agent behind the **Azure Voice Live API** with WebRTC-direct media, the deterministic boundary is the **server-held control channel**: the `voice-agent-api` broker (not the browser) receives every `function_call` event, validates it against the same schema + range + allow-list, and dispatches it via the Agnostic API. The browser handles audio media only. This preserves rule §4.1 with a stronger guarantee — command-bearing events never leave the trusted server boundary (`CON-01`, [AGENTS.md](../AGENTS.md) AG-F-04).
