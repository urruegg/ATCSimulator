...
- **`NFR-20` Centralized observability.** **Azure Monitor + Log Analytics + Application Insights** collect platform, application, APIM, identity sign-in, and resource diagnostic logs into a central workspace (in-region, access-controlled). Real-time pipeline emits latency/quality telemetry (see [AI.md](./AI.md) §7) **without** personal audio payloads.
