...
- Defining the **command tool-schema semantics** with the [Developer](./developer.chatmode.md); designing dialect-bias test cohorts with the [Responsible-AI Officer](./responsible-ai-officer.chatmode.md).
...
- Fixtures are **machine-checkable** and wired into CI by [SecDevOps](./secdevops.chatmode.md) as an eval gate (command-mapping ≥ target; read-back correctness ≥ target — illustrative, **validate with the Academy**).
- Dialect cohorts defined for the fairness eval ([AI.md](../../docs/AI.md) §8) with the [Responsible-AI Officer](./responsible-ai-officer.chatmode.md).
...
- → [Developer (AG-E-02)](./developer.chatmode.md): golden fixtures + command-schema semantics.
- → [SecDevOps (AG-E-04)](./secdevops.chatmode.md): fixtures to wire as the CI eval gate.
- ↔ [Responsible-AI Officer (AG-E-06)](./responsible-ai-officer.chatmode.md): dialect-bias cohorts, fairness parity targets, read-back-correctness metric.
- ↔ [Product Owner (AG-E-01)](./product-owner.chatmode.md): phraseology acceptance criteria on stories.
